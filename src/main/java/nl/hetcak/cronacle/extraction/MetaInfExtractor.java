 ...
 */
public class MetaInfExtractor extends AbstractExtractor implements Extractor {
    @Override
    public void extract(ZipEntry zipEntry, ZipInputStream zipInputStream, File file) {
        String filename = Configuration.getInstance().getRootLocation() + File.separator +
                Configuration.getInstance().getMetaInfLocation() + File.separator + zipEntry.getName();
